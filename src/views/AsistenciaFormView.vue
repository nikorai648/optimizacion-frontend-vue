...
import { getAsistencia, createAsistencia, updateAsistencia } from "../api/asistencias";
...
const validar = () => {
  if (!form.value.trabajador_rut || !form.value.trabajador_nombre) return "RUT y nombre del trabajador son obligatorios.";
...
const guardar = async (e) => {
  ...
  try {
    if (esEditar.value) await updateAsistencia(id, form.value);
    else await createAsistencia(form.value);
...
const cancelar = () => router.push("/asistencias");
</script>
  ...
  <div class="container mt-4 col-md-6">
    ...
    <form class="row g-3" @submit="guardar">
      ...
      <div class="col-md-3">
        ...
        <input type="time" v-model="form.hora_entrada" class="form-control" />
      </div>

      <div class="col-md-3">
        <label class="form-label">Hora salida</label>
        <input type="time" v-model="form.hora_salida" class="form-control" />
      </div>

      <div class="col-md-6">
        <label class="form-label">Tipo jornada</label>
        <select v-model="form.tipo_jornada" class="form-select">
