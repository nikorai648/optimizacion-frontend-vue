...
import { getDesempeno, createDesempeno, updateDesempeno } from "../api/desempenos";
...
const validar = () => {
  ...
  if (form.value.id_desempeno === "") {
    return "ID de desempeño obligatorio.";
  }
  return "";
};
...
const guardar = async (e) => {
  ...
  try {
    const payload = {
      ...form.value,
      id_desempeno: Number(form.value.id_desempeno),
    };
  ...
  <div class="container mt-4 col-md-6">
    ...
    <form class="row g-3" @submit="guardar">
      ...
      <div class="col-md-6">
        ...
        <input type="number" v-model="form.id_desempeno" class="form-control" required />
      </div>

      <div class="col-12">
        <label class="form-label">Forma de hacer trabajos</label>
        <textarea v-model="form.forma_de_hacer_trabajos" class="form-control" rows="2" />
      </div>

      <div class="col-12">
        <label class="form-label">Posibles quejas</label>
        <textarea v-model="form.posibles_quejas" class="form-control" rows="2" />
      </div>
