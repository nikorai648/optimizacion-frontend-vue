...
const validar = () => {
  if (!form.value.trabajador_rut || !form.value.trabajador_nombre) {
    return "RUT y nombre obligatorios.";
  }
  if (!form.value.mes) {
    return "Debe indicar el mes.";
  }
...
const guardar = async (e) => {
  ...
  try {
    const payload = {
      ...form.value,
      cantidad_trabajos_mes: Number(form.value.cantidad_trabajos_mes) || 0,
  ...
  <div class="container mt-4 col-md-6">
    ...
    <form class="row g-3" @submit="guardar">
      ...
      <div class="col-md-6">
        ...
        <input v-model="form.mes" class="form-control" placeholder="2025-11" required />
      </div>

      <div class="col-md-6">
        <label class="form-label">Cantidad trabajos</label>
        <input type="number" min="0" v-model="form.cantidad_trabajos_mes" class="form-control" />
      </div>

      <div class="col-12">
        <label class="form-label">Tipo trabajos</label>
        <input v-model="form.tipo_trabajos_mes" class="form-control" />
      </div>

      <div class="col-md-6">
        <label class="form-label">Sueldo total</label>
        <input type="number" min="0" v-model="form.sueldo_total_mes" class="form-control" />
      </div>

      <div class="col-md-6">
        <label class="form-label">ID Eficiencia (opcional)</label>
        <input type="number" v-model="form.id_eficiencia_asociada" class="form-control" />
      </div>
